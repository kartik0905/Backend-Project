...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  // We are not fetching the whole videos but a portion of it that's why we use paginated
...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  let videoFileUrl;
  // We are checking the thumbnail image file was uploaded or not
  if (
    req.files &&
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  if (!title && !description && !req.file) {
    throw new ApiError(
      400,
      "any of the title , description and thumbnail required to update"
    );
  }
